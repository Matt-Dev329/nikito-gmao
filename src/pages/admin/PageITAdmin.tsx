...
function TabDebugAssistant({ checks, edgeFns }: { checks: SystemCheck[]; edgeFns: EdgeFnStatus[] }) {
  ...
  const buildSystemContext = useCallback(() => {
    ...
    if (checks.length > 0) {
      ctx.sante_systeme = checks.map((c) => ({ check: c.label, status: c.status, detail: c.detail }));
    }
    if (edgeFns.length > 0) {
      ctx.edge_functions = edgeFns.map((f) => ({ name: f.name, status: f.status, latency_ms: f.latencyMs }));
    }
    ctx.navigateur = {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${supabaseUrl}/functions/v1/debug-assistant`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          history: updated.slice(0, -1).slice(-10),
        }),
      });
      ...
      if (data.success && data.reply) {
        ...
        setMessages((prev) => [...prev, {
          role: 'assistant',
          content: `Erreur : ${data.error ?? 'Reponse inattendue'}${data.detail ? '\n' + data.detail : ''}`,
        }]);
      }
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-280px)] min-h-[400px]">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={cn('flex gap-3', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
            ...
            <div className={cn(
              ...
              {msg.role === 'assistant' ? <MarkdownLite text={msg.content} /> : <span className="whitespace-pre-wrap">{msg.content}</span>}
            </div>
            {msg.role === 'user' && (
      ...
      {messages.length > 0 && (
        <button onClick={() => setMessages([])} className="mt-2 text-[11px] text-faint hover:text-dim self-center transition-colors">
