...
function buildHtml(parcsManquants: ParcManquant[], moisLabel: string): string {
  const lignesHtml = parcsManquants
    .map(
      ...
      <td style="padding:10px 14px;border-bottom:1px solid #1a1f4e;font-size:14px;color:#FF4D6D;font-weight:600;text-align:center;">A faire</td>
    </tr>`,
    )
    .join("\n");
...
<title>Rappel controle mensuel</title>
</head>
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td style="padding-bottom:8px;">
                  <span style="display:inline-block;background-color:#FFB400;color:#0B0B2E;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;padding:4px 10px;border-radius:6px;">Rappel</span>
                </td>
              </tr>
              <tr>
                <td style="font-size:18px;color:#ffffff;font-weight:600;padding-bottom:6px;">
                  Controle mensuel a planifier
                </td>
              </tr>
              <tr>
                <td style="font-size:13px;color:#8b92b8;padding-bottom:24px;">
                  Periode : <strong style="color:#ffffff;">${moisLabel}</strong> &mdash; ${parcsManquants.length} parc${parcsManquants.length > 1 ? "s" : ""} concern&eacute;${parcsManquants.length > 1 ? "s" : ""}
                </td>
              </tr>
              <tr>
                <td>
              ...
              <tr>
                <td style="padding-top:24px;text-align:center;">
                  <a href="https://nikito.tech/tech/controle-mensuel" style="display:inline-block;background-color:#5DE5FF;color:#0B0B2E;font-size:13px;font-weight:700;text-decoration:none;padding:12px 28px;border-radius:10px;">Faire le controle</a>
                </td>
              </tr>
            </table>
          </td>
  ...
  try {
    ...
    const [parcsRes, ctrlRes] = await Promise.all([
      ...
      supabase
        ...
        .select("parc_id")
        .eq("type", "mensuel")
        .gte("date_planifiee", debutMois)
        .lte("date_planifiee", finMois)
        .eq("statut", "valide"),
    ]);
    ...
    if (parcsManquants.length === 0) {
      return new Response(
        JSON.stringify({ success: true, message: "Tous les controles mensuels sont faits", parcs_manquants: 0 }),
    ...
    const html = buildHtml(parcsManquants, moisLabel);
    const subject = `[RAPPEL] Controle mensuel ${moisLabel} - ${parcsManquants.length} parc${parcsManquants.length > 1 ? "s" : ""} a planifier`;
    ...
    console.error("rappel-controle-mensuel error:", err);
    return new Response(
