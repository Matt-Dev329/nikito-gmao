/*
  # Explicitly set security_invoker on vue_recurrences_actives

  1. Changes
    - Recreate `vue_recurrences_actives` with explicit `security_invoker = true`
    - This resolves the Supabase linter warning about SECURITY DEFINER

  2. Security
    - View now explicitly runs with the calling user's permissions
    - RLS policies on underlying tables are properly enforced
*/

DROP VIEW IF EXISTS vue_recurrences_actives;

CREATE VIEW vue_recurrences_actives
