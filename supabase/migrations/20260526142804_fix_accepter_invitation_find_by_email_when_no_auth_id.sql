/*
  # Fix accepter_invitation: find existing user by email even when p_auth_user_id is null

  1. Problem
    - When re-inviting an existing PIN-only user, p_auth_user_id is null
    - The existing logic only searched by email when p_auth_user_id IS NOT NULL
    - This caused a duplicate key violation on utilisateurs.email_key

  2. Fix
    - Always search by email (v_invit.email) regardless of p_auth_user_id value
    - If found, UPDATE existing row instead of INSERT (existing behavior)
    - Also update code_pin_hash when a new PIN is provided during re-invitation
*/
...
BEGIN
  ...
  ELSE
    UPDATE utilisateurs SET
      ...
      actif = true,
      code_pin_hash = CASE WHEN p_pin_clair IS NOT NULL THEN crypt(p_pin_clair, gen_salt('bf')) ELSE utilisateurs.code_pin_hash END,
