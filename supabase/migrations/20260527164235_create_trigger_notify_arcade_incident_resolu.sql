/*
  # Notification email quand un incident arcade est resolu

  1. Nouvelle fonction trigger
    - `fn_notify_arcade_incident_resolu()` : detecte quand un incident arcade
      passe au statut "resolu" et appelle l'edge function
      `notify-arcade-incident-resolu` via pg_net pour envoyer un email
      a joachim.miloche@ja-fg.com et lilimarie.pellerin@ja-fg.com

  2. Nouveau trigger
    - `trg_notify_arcade_incident_resolu` sur UPDATE de `statut` dans `incidents`
    - Se declenche AFTER UPDATE FOR EACH ROW
    - Condition : ancien statut != 'resolu' ET nouveau statut = 'resolu'
...
-- Creer le trigger
DROP TRIGGER IF EXISTS trg_notify_arcade_incident_resolu ON incidents;

CREATE TRIGGER trg_notify_arcade_incident_resolu
  AFTER UPDATE OF statut ON incidents
  FOR EACH ROW
  EXECUTE FUNCTION fn_notify_arcade_incident_resolu();
